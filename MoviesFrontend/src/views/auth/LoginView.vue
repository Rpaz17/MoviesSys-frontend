...
import { AlertCircle, ArrowRight, Eye, EyeOff, Film, Loader2, XCircle } from "lucide-vue-next";
import { useSessionStore } from "../../stores/session";
import { vEmail, vPassLogin } from "../../lib/validation";
...
async function submit() {
  ...
  const ok = await session.login(form.email, form.password);
  status.value = ok ? "idle" : "error";
  if (ok) {
    router.push(session.isAdmin ? "/admin" : redirectTo.value);
  } else {
  ...
  <div class="auth-shell">
    ...
    <div class="auth-card">
      ...
      <div class="auth-footer">
        <button class="auth-link" @click="router.push({ path: '/register', query: route.query })">Crear cuenta</button>
        <button class="auth-link auth-link-muted" @click="router.push('/forgot')">Olvide mi contrasena</button>
      </div>
    </div>
  </div>
</template>

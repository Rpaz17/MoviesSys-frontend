...
export const vName = (v: string) =>
  ...
  v.trim().length < 2 ? "Minimo 2 caracteres." :
  v.trim().length > 60 ? "Maximo 60 caracteres." : "";

export const vEmail = (v: string) =>
  !v ? "El correo es obligatorio." :
  !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) ? "Ingresa un correo valido." : "";

export const vPass = (v: string) =>
  !v ? "La contrasena es obligatoria." :
  v.length < 8 ? "Minimo 8 caracteres." :
  !/[A-Z]/.test(v) ? "Debe contener al menos una mayuscula." :
  !/[0-9]/.test(v) ? "Debe contener al menos un numero." : "";

export const vPassLogin = (v: string) =>
  !v ? "La contrasena es obligatoria." :
  v.length < 6 ? "Minimo 6 caracteres." : "";

export const vConfirm = (p: string, c: string) =>
  !c ? "Confirma tu contrasena." :
  p !== c ? "Las contrasenas no coinciden." : "";

export const vPhone = (v: string) =>
  !v ? "" :
  !/^[+]?[\d\s\-().]{7,20}$/.test(v) ? "Numero de telefono invalido." : "";
